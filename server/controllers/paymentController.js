...
const processPayment = async (req, res) => {
  try {
    ...
    console.log('User ID comparison:', appointment.patient.toString(), userId.toString());
    ...
    if (!validPaymentStatuses.includes(appointment.status)) {
      console.log('Appointment status does not allow payment:', appointment.status);
      return res.status(400).json({ success: false, message: 'Appointment must be confirmed before payment' });
    }
    ...
    if (existingPayment) {
      ...
      if (existingPayment.status === 'completed') {
        ...
        return res.status(400).json({ success: false, message: 'Payment already completed for this appointment' });
      }
...
const downloadReceipt = async (req, res) => {
  try {
    ...
    res.json({
      ...
      receiptUrl: payment.receiptUrl || `https://api.vitala.com/receipts/${payment.receiptNumber}.pdf`
    });
  } catch (error) {
