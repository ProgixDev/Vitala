...
/** Localized display name for a service (falls back to the raw name). */
export function getServiceLabel(service: Pick<Service, "category" | "name">): string {
  return t(`service.${service.category}`) || service.name;
}

export function getServiceLabelById(id: string): string {
  const service = servicesData.find((s) => s._id === id);
  return service ? getServiceLabel(service) : "Unknown Service";
}
